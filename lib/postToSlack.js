...
module.exports = db => async function postToSlack(pkg, newVersions) {
  ...
  const accounts = await Account.
    find({ packagesWatched: pkg._id, type: 'SLACK' });
  for (const account of accounts) {
    const customer = await db.model('Customer').findOne({ accountIds: account._id });
    if (!customer || !customer.stripe.customerId) {
      continue;
    }
...
async function _postToSlack(webhook, pkg, version, url, changelog, githubUrl) {
  ...
  if (changelog != null) {
    ...
    text = `${pkg} v${version} released. <${npmUrl + '/package/' + pkg}|See npm for more details>`;
  }
