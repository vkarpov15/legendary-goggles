...
module.exports = db => async function updatePackage(id) {
  ...
  const npmData = await http.get(`https://${server}/${id}`).
    catch(error => {
  ...
  // Perf optimization because npm basically made every single package modified
  // around Jan 1, 2019.
  const time = npmData.time != null && npmData.time.modified != null ? new Date(npmData.time.modified) : null;
  if (time == null || time.valueOf() < new Date('2018-01-25').valueOf()) {
    return { pkg: null, newVersions: [] };
  }
